...
(function () {
  ...
  if (window.authManager && !window.authManager.isLoggedIn()) {
    alert('Please login to track your laundry.');
  ...
  class TrackingDisplay {
    constructor(tracker) {
      ...
      this.createMySubmissionsSection();
    }

    createMySubmissionsSection() {
      ...
      if (statusSection && window.authManager) {
        ...
        if (currentUser) {
          ...
          this.updateMySubmissions(currentUser.studentId);
        }
      }
    }

    updateMySubmissions(studentId) {
  ...
  class LaundryTrackingApp {
    ...
    async init() {
      try {
        ...
        // Update "My Submissions" section with current user's data
        if (window.authManager) {
          const currentUser = window.authManager.getCurrentUser();
          if (currentUser) {
            this.display.updateMySubmissions(currentUser.studentId);
          }
        }
    ...
    async loadTrackingData() {
      try {
        ...
        const response = await fetch('assests/data/laundry-tracking.json');
        if (!response.ok) {
          throw new Error('Failed to load tracking data');
        }
        const jsonData = await response.json();
        
        // Then, get user submissions from localStorage
        const submissions = JSON.parse(localStorage.getItem('laundryBuddy_submissions') || '[]');
        
        // Combine JSON data with user submissions
        jsonData.laundryItems = [...jsonData.laundryItems, ...submissions];
        
        return jsonData;
      } catch (error) {
        console.error('Error loading tracking data:', error);
        // Try to load only from localStorage if JSON fails
